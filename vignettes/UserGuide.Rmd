...
Plot power vs. minimum detectable percent change over a range of nA values

```{r}
inputGrid<-expand.grid(nA = 1:40, percent_change = seq(0,50,5))
inputGrid$power <- mapply(function(nA, pct) {
  find_power_for_percent_change(
    S = S_demo,
    nB = nB_demo,
    nA = nA,
    sd_within = sd_within_hat,
...
ggplot(inputGrid, aes(x = percent_change, y = power, color = factor(nA))) +
  geom_line() +
  geom_hline(yintercept = 0.8, linetype = "dashed", color = "black") +
  labs(
    title = "Power vs. Percent Change for Different After Sample Sizes",
    x = "Percent Change",
    y = "Power",
    color = "After Samples (nA)"
  ) +
```
