...
logsd_within <- fishSum %>%
  ...
  pull(logsd_within)
site_number<-52
#Mean before in original units for percent conversion.
...
power_values <- sapply(nA_grid, function(nA) {
  power_for_nA_analytical(nA, S = site_number, 
               nB = nB, 
               delta = delta_target_log,
...
inputGrid<-expand.grid(nA = 2:10, percent_change = seq(5,50,5))
...
min_detectable_pct <- percent_change_res
